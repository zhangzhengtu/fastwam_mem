"""
RoboTwin-Mem single-task evaluation entrypoint (Hydra).

Features:
- Read `configs/sim_robotwin.yaml`.
- Copy FastWAM policy into RoboTwin-Mem:
  `experiments/robotwin_mem/fastwam_policy -> RoboTwin-Mem/policy/fastwam_policy`.
- Forward config overrides to the RoboTwin-Mem entrypoint
  `script/eval_policy.py` and save logs.
- RoboTwin-Mem only runs `demo_clean`.
...
def _get_task_override_keys() -> set[str]:
    try:
        overrides = HydraConfig.get().overrides.task
    except Exception:
        return set()
    return {ov.split("=", 1)[0].lstrip("+~") for ov in overrides}


def _resolve_mem_task_config(cfg: DictConfig) -> str:
    override_keys = _get_task_override_keys()
    requested = str(cfg.EVALUATION.task_config)
    if "EVALUATION.task_config" in override_keys and requested != DEMO_CLEAN_TASK_CONFIG:
        raise ValueError(
            "RoboTwin-Mem evaluation only supports "
            f"`EVALUATION.task_config={DEMO_CLEAN_TASK_CONFIG}`, got: {requested}"
        )
    return DEMO_CLEAN_TASK_CONFIG


def _ensure_policy_copy(robotwin_root: Path, policy_source_dir: Path) -> Path:
    ...
    if not source_resolved.is_dir():
        raise FileNotFoundError(f"Policy source directory not found: {source_resolved}")

    if policy_target.is_symlink():
        policy_target.unlink()

    if policy_target.exists() and not policy_target.is_dir():
        raise RuntimeError(
            f"Path already exists and is not a directory: {policy_target}. "
            "Please handle it manually to avoid overriding an unexpected file."
        )

    # Some object-store backed filesystems do not support symlinks. Keep the
    # RoboTwin policy import path stable by copying the adapter directory.
    shutil.copytree(source_resolved, policy_target, dirs_exist_ok=True)
...
def main(cfg: DictConfig):
    ...
    policy_source_dir = (PROJECT_ROOT / "experiments" / BENCH_NAME / POLICY_NAME).resolve()
    if not policy_source_dir.is_dir():
        raise FileNotFoundError(f"Policy source directory not found: {policy_source_dir}")

    _ensure_policy_copy(robotwin_root=robotwin_root, policy_source_dir=policy_source_dir)
    ...
    task_config = _resolve_mem_task_config(cfg)
    ...
    if return_code != 0:
        raise RuntimeError(f"RoboTwin-Mem evaluation failed with return code {return_code}. Log: {log_file}")
