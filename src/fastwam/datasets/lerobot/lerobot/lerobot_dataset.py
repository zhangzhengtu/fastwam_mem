...
class LeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        timestamp_col = self.hf_dataset["timestamp"]
        timestamps = np.asarray(
            [x.item() if hasattr(x, "item") else x for x in timestamp_col],
            dtype=np.float64,
        )
        ...
        episode_indices = np.asarray(
            [x.item() if hasattr(x, "item") else x for x in episode_indices_col],
            dtype=np.int64,
        )
    ...
    @staticmethod
    def _stack_column_values(values) -> torch.Tensor:
        values = list(values)
        if len(values) > 0 and not torch.is_tensor(values[0]):
    ...
    def _get_query_timestamps(
        ...
        for key in self.meta.video_keys:
            if query_indices is not None and key in query_indices:
                ...
                query_timestamps[key] = self._stack_column_values(timestamps).tolist()
            else:
    ...
    def _query_hf_dataset(self, query_indices: dict[str, list[int]]) -> dict:
        return {
            key: self._stack_column_values(self.hf_dataset.select(q_idx)[key])
    ...
    def _query_hf_dataset_fast(self, query_indices: dict[str, list[int]]) -> dict:
        ...
        for key, q_idx in query_indices.items():
            if key not in self.meta.video_keys :
                ...
                result[key] = self._stack_column_values(selected_data[key])
        return result
    ...
    def get_episode_data(self, episode_id: int) -> dict:
        ...
        res = {key : self._stack_column_values(selected_data[key]) for key in res_keys}
        return res
