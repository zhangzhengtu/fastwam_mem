...
class BaseLerobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self._build_episode_metadata_cache()
    ...
    def _build_episode_metadata_cache(self):
        self.trajectory_ids: list[int] = []
        self.trajectory_lengths: list[int] = []
        self.trajectory_start_indices: list[int] = []
        self.episode_to_dataset_index: list[int] = []
        self.episode_to_local_episode_index: list[int] = []
        self.keyframe_steps_by_episode: dict[int, list[int]] = {}
        self.inspect_keyframe_steps_by_episode: dict[int, list[int]] = {}
        self._dataset_episode_to_trajectory_id: dict[tuple[int, int], int] = {}
        ...
        for dataset_index, dataset in enumerate(self.multi_dataset._datasets):
            ...
            if selected_episodes is None:
                selected_episodes = list(range(dataset.num_episodes))

            for selected_pos, local_episode_index in enumerate(selected_episodes):
                episode_info = dataset.meta.episodes[int(local_episode_index)]
                length = int(episode_info["length"])
                start = int(dataset.episode_data_index["from"][selected_pos].item()) + global_start

                self.trajectory_ids.append(trajectory_id)
                self.trajectory_lengths.append(length)
                self.trajectory_start_indices.append(start)
                self.episode_to_dataset_index.append(int(dataset_index))
                self.episode_to_local_episode_index.append(int(local_episode_index))
                self.keyframe_steps_by_episode[trajectory_id] = self._as_int_list(
                    episode_info.get("keyframe_steps")
                )
                self.inspect_keyframe_steps_by_episode[trajectory_id] = self._as_int_list(
                    episode_info.get("inspect_keyframe_steps")
                )
                self._dataset_episode_to_trajectory_id[(int(dataset_index), int(local_episode_index))] = trajectory_id
                trajectory_id += 1

            global_start += int(dataset.num_frames)

        self.max_keyframe_steps_per_episode = max(
            (len(v) for v in self.keyframe_steps_by_episode.values()),
            default=0,
        )
        self.max_inspect_keyframe_steps_per_episode = max(
            (len(v) for v in self.inspect_keyframe_steps_by_episode.values()),
            default=0,
        )
    ...
    def get_keyframe_steps(self, trajectory_id: int) -> list[int]:
        return list(self.keyframe_steps_by_episode.get(int(trajectory_id), []))

    def get_inspect_keyframe_steps(self, trajectory_id: int) -> list[int]:
        return list(self.inspect_keyframe_steps_by_episode.get(int(trajectory_id), []))

    def has_inspect_keyframe_annotations(self) -> bool:
        return any(len(v) > 0 for v in self.inspect_keyframe_steps_by_episode.values())

    def is_inspect_keyframe(self, trajectory_id: int, timestep: int) -> bool:
        return int(timestep) in set(self.get_inspect_keyframe_steps(trajectory_id))

    def episode_step_to_global_index(self, trajectory_id: int, step_index: int) -> int:
        trajectory_id = int(trajectory_id)
        if trajectory_id < 0 or trajectory_id >= len(self.trajectory_start_indices):
            raise IndexError(f"trajectory_id {trajectory_id} out of bounds.")
        step_index = int(step_index)
        length = int(self.trajectory_lengths[trajectory_id])
        if step_index < 0 or step_index >= length:
            raise IndexError(
                f"step_index {step_index} out of bounds for trajectory {trajectory_id} length {length}."
            )
        return int(self.trajectory_start_indices[trajectory_id] + step_index)

    def global_index_to_trajectory_step(self, global_index: int) -> tuple[int, int]:
        global_index = int(global_index)
        if global_index < 0 or global_index >= len(self):
            raise IndexError(f"Index {global_index} out of bounds {len(self)}.")
        starts = self.trajectory_start_indices
        lo, hi = 0, len(starts)
        while lo + 1 < hi:
            mid = (lo + hi) // 2
            if starts[mid] <= global_index:
                lo = mid
            else:
                hi = mid
        return lo, global_index - int(starts[lo])

    def dataset_episode_to_trajectory_id(self, dataset_index: int, episode_index: int) -> int:
        key = (int(dataset_index), int(episode_index))
        if key not in self._dataset_episode_to_trajectory_id:
            raise KeyError(f"Unknown dataset/episode pair: {key}")
        return int(self._dataset_episode_to_trajectory_id[key])
    ...
    def __getitem__(self, idx):
            ...
            try:
                ...
                if self.allow_random_fallback_on_error:
                    logger.warning(
                        ...
                        "Retrying with a random index because `allow_random_fallback_on_error=True`. "
                        f"Error: {err}"
                    )
                    ...
                    logger.warning(
                        f"Error loading sample {sample_idx} (attempt {attempt}). "
                        "Retrying the same index to preserve episode/sample alignment. "
                        f"Error: {err}"
                    )
        ...
        if "dataset_index" in metadata and "episode_index" in metadata:
            try:
                dataset_index = self._as_int(metadata["dataset_index"])
                episode_index = self._as_int(metadata["episode_index"])
                trajectory_id = self.dataset_episode_to_trajectory_id(dataset_index, episode_index)
                metadata["trajectory_id"] = torch.tensor(trajectory_id, dtype=torch.long)
            except KeyError:
                pass
        if "frame_index" in metadata and "timestep" not in metadata:
            metadata["timestep"] = metadata["frame_index"]
