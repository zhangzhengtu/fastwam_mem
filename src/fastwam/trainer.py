...
from .utils.sequence_sampler import SequentialEpisodeSampler
...
class Wan22Trainer:
    ...
    def _build_loader(self, dataset, worker_init_fn=None):
        ...
        if sampler_type == "sequential_episode":
            action_horizon = getattr(dataset, "action_horizon", None)
            if action_horizon is None:
                action_horizon = int(getattr(dataset, "num_frames")) - 1
            episode_dataset = getattr(dataset, "lerobot_dataset", dataset)
            sample_stride = int(getattr(episode_dataset, "global_sample_stride", 1))
            self.train_sampler = SequentialEpisodeSampler(
                dataset=dataset,
                seed=self.seed,
            ...
            raise ValueError(
                f"Unsupported sampler.type={sampler_type!r}. Expected 'random' or 'sequential_episode'."
            )
        return DataLoader(
            dataset,
            batch_size=self.batch_size,
    ...
    def _estimate_total_train_steps(self) -> int:
        ...
        sample_count = len(self.train_sampler) if hasattr(self, "train_sampler") else len(self.train_dataset)
        micro_steps_per_epoch = max(ceil(sample_count / global_batch_size), 1)
    ...
    def _apply_dit_only_train_mode(model):
        ...
        kem_head = getattr(model, "kem_head", None)
        if kem_head is not None:
            kem_head.train()
