    ...
    def forward(
        ...
        if attention_mask.ndim == 2:
            ...
            if attention_mask.shape[1] != 1:
                raise ValueError(
                    f"4D `attention_mask` must have shape [B,1,S,S], got {tuple(attention_mask.shape)}"
                )
        ...
        for layer_idx in range(self.num_layers):
            ...
            mask_seq = attention_mask.shape[0] if attention_mask.ndim == 2 else attention_mask.shape[-1]
            if mask_seq != total_seq:
                raise ValueError(
