...
class FastWAMIDM(FastWAMJoint):
    ...
    def _build_teacher_forcing_attention_mask(
        ...
        if cond_memory_seq_len > 0:
            ...
            if cond_normal_seq_len <= 0:
                raise ValueError(
                    "Memory prefix leaves no normal cond-video tokens: "
                    f"cond_video_seq_len={cond_video_seq_len}, cond_memory_seq_len={cond_memory_seq_len}."
                )
        else:
            cond_normal_seq_len = int(cond_video_seq_len)
        ...
        if cond_memory_seq_len > 0:
            cond_mask = torch.zeros((cond_video_seq_len, cond_video_seq_len), dtype=torch.bool, device=device)
            cond_normal_mask = self.video_expert.build_video_to_video_mask(
                video_seq_len=cond_normal_seq_len,
                video_tokens_per_frame=cond_video_tokens_per_frame,
                device=device,
            )
            first_frame_tokens = min(cond_video_tokens_per_frame, cond_normal_seq_len)
            condition_end = cond_memory_seq_len + first_frame_tokens
            cond_mask[:condition_end, :condition_end] = True
            cond_mask[cond_memory_seq_len:cond_video_seq_len, cond_memory_seq_len:cond_video_seq_len] = cond_normal_mask
            cond_mask[condition_end:cond_video_seq_len, :cond_memory_seq_len] = True
            mask[noisy_end:cond_end, noisy_end:cond_end] = cond_mask
        else:
    ...
    def infer_joint(
        ...
        for step_t_action, step_delta_action in zip(infer_timesteps_action, infer_deltas_action):
            ...
            if self.kem_enabled:
                pred_action, last_action_tokens = pred_action_result
            else:
                pred_action = pred_action_result
